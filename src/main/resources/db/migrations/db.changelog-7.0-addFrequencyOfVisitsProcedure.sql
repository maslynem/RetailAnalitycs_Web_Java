...
CREATE OR REPLACE FUNCTION get_frequency_of_visits (first_date TIMESTAMP, last_date TIMESTAMP, added_transactions BIGINT,
                                                    maximum_churn_index DOUBLE PRECISION, maximum_share_of_transactions DOUBLE PRECISION, allowable_margin_share DOUBLE PRECISION)
    RETURNS TABLE (
                      Customer_ID BIGINT,
    ...
    RETURN QUERY
        SELECT DISTINCT
            ...
            round((abs(EXTRACT(EPOCH FROM first_date - last_date) / 86400)) /
                  (SELECT customer_frequency FROM customers WHERE customers.customer_id = groups."Customer_ID"))::DOUBLE PRECISION + added_transactions,
