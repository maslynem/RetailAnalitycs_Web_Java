...
WITH new_purchase_history AS (SELECT cards.Customer_ID                 AS Customer_ID,
     ...
     stab AS (SELECT tmp.Customer_ID,
                     tmp.Group_ID,
                     avg(abs(diff - p2.Group_Frequency) / p2.Group_Frequency)::DOUBLE PRECISION AS Group_Stability_Index
...
SELECT p.Customer_ID,
       p.Group_ID,
       CAST(p.Group_Purchase AS DOUBLE PRECISION) / aff_ind.tr_count    AS Group_Affinity_Index,
       ...
       p.Group_Frequency::DOUBLE PRECISION                              AS Group_Churn_Rate,
