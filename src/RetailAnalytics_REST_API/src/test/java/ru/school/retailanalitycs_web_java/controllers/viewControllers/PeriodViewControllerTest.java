...
class PeriodViewControllerTest extends IntegrationTestBase {
    private static final PeriodViewId PERIOD_VIEW_ID = new PeriodViewId(Customer.builder().id(4L).build(), SkuGroup.builder().id(7L).build());
    private static final PeriodViewId NOT_EXISTING_PERIOD_VIEW_ID = new PeriodViewId(Customer.builder().id(Long.MAX_VALUE).build(), SkuGroup.builder().id(Long.MAX_VALUE).build());
    ...
    void findPeriodById() throws Exception {
        PeriodViewDto first = PeriodViewDto.builder()
                .customer(getCustomerDtoWithId(PERIOD_VIEW_ID.getCustomer().getId()))
                .skuGroup(getSkuGroupDtoWithId(PERIOD_VIEW_ID.getSkuGroup().getId()))
                .groupPurchase(1L)
                .groupFrequency(1.0)
                .groupMinDiscount(0.4335659145964605)
                .build();

        mockMvc.perform(get("/api/v1/views/periods/{customer_id}/{group_id}", PERIOD_VIEW_ID.getCustomer().getId(), PERIOD_VIEW_ID.getSkuGroup().getId()))
                .andExpect(status().isOk())
    ...
    void findNotExistingPeriod() throws Exception {
        mockMvc.perform(get("/api/v1/views/periods/{customer_id}/{group_id}",
                        NOT_EXISTING_PERIOD_VIEW_ID.getCustomer().getId(), NOT_EXISTING_PERIOD_VIEW_ID.getSkuGroup().getId()))
